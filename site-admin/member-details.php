    ...
    var f = $('<div></div>').elfinder({
	   ...
	   <script>
	    	...
	    	function insertrecord()
	    		{  
		    		$.post("member-update.php?update=2", {id:$('#id').val(), fullname:$('#fullname').val(),email:$('#email').val(),company:$('#company').val(),designation:$('#designation').val(),address:$('#address').val(),phone:$('#phone').val(),zip:$('#zip').val(),city:$('#city').val(),country:$('#country').val(),profile:$('#profile').val(),memrole:$('#memrole').val(), url1:$('#url1').val(), url2:$('#url2').val() },
								function(data) 
								{
