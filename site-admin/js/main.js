...
function deleteme() 
	    		{
		    		$.post("delrecord.php", {id:$('#delid').val(), tabname:$('#deltablename').val(),rolid:$('#rolid').val(),sesid:$('#sesid').val()},
								function(data) 
								{
								alert("Deleted!");
...
function confirmbox(tabname, id, ses, rol) 
	    	{ 
	    		ses = ses || 0;
	    		rol = rol || 0;
	    		$('#deltablename').val(tabname);
	    		$('#delid').val(id);
	    		$('#rolid').val(rol);
	    		$('#sesid').val(ses);
	    		$('#deletebox').trigger('click'); 
	    	}
